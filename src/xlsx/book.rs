...
<cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles>
            ...
            let mut shared_strings: Arc<Mutex<Xml>> =
                Arc::new(Mutex::new(Xml::new(&String::new())));
                ...
                match name_str {
                    ...
                    s if s.ends_with(XML_SUFFIX) => {
                        ...
                        match s {
                            ...
                            WORKBOOK_FILENAME => { workbook = xml; }
                            STYLES_FILENAME => { styles = Arc::new(Mutex::new(xml)); }
                            SHARED_STRINGS_FILENAME => { shared_strings = Arc::new(Mutex::new(xml)); }
                            _ => {}
                        }
                    }
                    // リレーションシップファイル
                    s if s.ends_with(XML_RELS_SUFFIX) => {
                        let mut contents = String::new();
                        file.read_to_string(&mut contents).unwrap();
                        let xml = Xml::new(&contents);
                        match s {
...
impl Book {
    ...
    pub fn get_sheet_paths(&self) -> HashMap<String, String> {
        ...
        let sheet_paths: HashMap<String, String> = relationships
            .into_iter()
            .filter_map(|x| {
                if let (Some(id), Some(target)) = (x.attributes.get("Id"), x.attributes.get("Target")) {
        ...
        for sheet_tag in sheet_tags {
            if let (Some(id), Some(name)) = (sheet_tag.attributes.get("r:id"), sheet_tag.attributes.get("name")) {
                if let Some(sheet_path) = sheet_paths.get(id) {
                    let trimmed_path = sheet_path
                        .trim_start_matches("/xl/")
                        .trim_start_matches("xl/");
                    result.insert(name.clone(), format!("xl/{trimmed_path}"));
                }
            }
        }
        result
